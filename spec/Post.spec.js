...
describe("Tests du contrôleur PostCtrl", () => {
    ...
    describe("getPostById", () => {
        it("devrait renvoyer un post existant", async () => {
            req.params = { id: 1 };
            const mockPost = { id: 1, title: "Test Post", description: "Post description" };
            spyOn(prisma.post, "findUnique").and.returnValue(Promise.resolve(mockPost));
        ...
        it("devrait retourner une erreur 404 si le post n'existe pas", async () => {
            ...
            expect(res.status().json).toHaveBeenCalledWith({ message: "Post not found" });
        });
    });

    describe("getAllPosts", () => {
        it("devrait renvoyer la liste des posts", async () => {
            const mockPosts = [{ id: 1, title: "Test Post", description: "Post description" }];
            spyOn(prisma.post, "findMany").and.returnValue(Promise.resolve(mockPosts));
    ...
    describe("createPost", () => {
        it("devrait créer un nouveau post", async () => {
            ...
            spyOn(prisma.post, "create").and.returnValue(Promise.resolve(mockNewPost));
    ...
    describe("updatePost", () => {
        it("devrait mettre à jour un post existant", async () => {
            ...
            spyOn(prisma.post, "update").and.returnValue(Promise.resolve(mockUpdatedPost));
        ...
        it("devrait retourner une erreur 404 si le post n'est pas trouvé", async () => {
            req.params = { id: 999 };
            req.body = { title: "Non Existant Post", description: "This post does not exist" };
            ...
            expect(res.status).toHaveBeenCalledWith(404);
            expect(res.status().json).toHaveBeenCalledWith({ error: "Post not found" });
        });
    });
    ...
    describe("deletePost", () => {
        it("devrait supprimer un post existant", async () => {
            ...
            spyOn(prisma.post, "delete").and.returnValue(Promise.resolve(mockDeletedPost));
